...
however, these commands are valid for Unix, but
for Windows, where Larry advises Strawberry perl
we have to run 
=item C<dmake test; dmake dist>

and B<dmake dist> will be executed, if we install gnu-tar and gnu-zip
 and add the path to them in the %PATH%
