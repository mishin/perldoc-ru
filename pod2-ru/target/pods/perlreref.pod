...
применяя соответствующие флаги (опции).
    ...
    x  Улучшение читаьельности (eXtended legibility) - свободно ставим пробелы и комментарии
...
Разделители отличные от '/' могут использоваться для этого оператора и следующие из них.
Delimiters other than '/' may be used for both this
operator and the following ones. Ведущую  C<m> моржно опустить, если разделитель '/'.
Если 'образец' - пустая последовательность, последнее успешно соответствовало regex, используется.
