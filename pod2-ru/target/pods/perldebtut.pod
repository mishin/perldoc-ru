...
perldebtut - Учебник по отладке в Perl

=head1 ОПИСАНИЕ

Это очень упрощенное введение в использование отладчика perl  и
указатель на существующие, более подробные источники информации по вопросам отладки
программ на Perl.

Существует чрезвычайно большое число людей, которые не представляют себе ничего
...
простой, когда дело доходит до отладки perl программ, без использования
отладчика вообще. Чтобы это продемонстрировать, вот простой скрипт с именем "hello",
с проблемой:
...
Хотя это компилируется и выполняет успешно, он вероятно не будет делать то, что ожидается,
а именно он совсем не печатает  "Hello World\n"; c другой стороны она делает
именно то, что было приказано, компьютеры, обычно выполняют биты, которые им передают.
Скрипт напечатает знак новой строки, и вы получите то, что выглядит как
пустая строка. Здесь мы видими 2 переменные, тогда как их (из-за опечатки)
на самом деле 3:
...
Чтобы поймать такого рода проблемы, мы должны заставить объявить каждую переменную
перед использованием, поставив в начале модуль strict, поставив 'use strict;' после
первой строки скрипта.

Теперь, когда вы запускаете его, perl жалуется о 3-х необъявленных переменных и мы
получим четыре сообщения об ошибках, потому что одна переменная используется дважды:
...
Супер! и чтобы это исправить мы объявим все переменные явно и теперь наш
...
Потом мы делаем (всегда хорошая идея) проверку синтаксиса, прежде чем мы попытаемся запустить его снова:

	> perl -c hello
	hello syntax OK 

И теперь когда мы запускаем его, мы получаем «\n», но мы, по крайней мере, знаем почему. Просто
полученный скрипт во время компиляции выявил переменную '$varl' (с буквы 'l')
, которая просто меняется $varl на $var1 и это решает проблему.

=head1 Смотрим на данные -w и v

Хорошо, но как насчет того, когда вы действительно хотите увидеть ваши данные, что там, в этой
динамической переменной перед тем, как ее использовать?
...
Выглядит хорошо, после того, как он был проведен через проверку синтаксиса (perl -c scriptname), мы
pапустили его и получаем пустую строку снова! Хм-м.

Один общий подход для отладки такой, чтобы использовать выражение print 
для вывода тех переменных, которые мы хотим проверить перед тем как 
мы выводи наши данные, и теперь после:
...
После долгих минут взгляда на этоте кусок кода и не видя леса за деревьями
некоторое время, мы идем пить кофе и попробуем другой подход. Мы используем 
 кавалерию, давая perl ключ командной строки 'B<-d>':
...
Теперь, то, что мы сделали здесь это запуск встроенного отладчика  perl для нашего
сценария. Он останавливается на первой строке исполняемого кода и ждет
входных данных.

Прежде, чем мы пойдем дальше, вы захотите знать, как выйти из отладчика: нужно просто использовать
 букву 'B<q>', а не слова 'quit' или 'exit':
	
	DB<1> q
	>

Вот-вот, ты снова вернулся на родную землю.
