...
=for comment
Этот документ в в формате Pod.  Для его чтения используйте средство форматирования Pod такое, как
 "perldoc perlpod".

=head1 НАИМЕНОВАНИЕ
X<POD> X<plain old documentation>

perlpod - формат простой старой документации   (Plain Old Documentation format)

=head1 ОПИСАНИЕ

Pod - это простой-в-использовании язык разметки для написания документации для Perl, 
Perl программ, и Perl модулей.

Доступны программы для преобразования Pod в различные форматы:
обычный текст, HTML, man-страницы и многие другие.

Pod разметка состоит из трех основных видов пунктов:
L<ordinary|/"Обычный параграф">,
L<verbatim|/"Стенографические параграф">, и 
L<command|/"Командный параграф">.


=head2 Обычный параграф
