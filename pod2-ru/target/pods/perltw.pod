...
如果你用一般的文字編輯器閱覽這份文件, 請忽略文中奇特的註�?字符.
這份文件�?�以 POD (簡�?�文件格式) 寫成; 這種格式�?�為了能讓人直接讀取,
...
也連帶支援了許多拉丁語系以外的編碼方式; CJK (中日韓) 便�?�其中的一部份.
Unicode �?�國際性的標準, 試圖涵蓋世界上所有的字符: 西方世界, 東方世界,
以及兩者間的一切 (希�?文, �?利亞文, �?�拉伯文, 希伯來文, 印度文,
...
在輸入及輸出時, 為了處理以 Unicode 之前的編碼方式儲�?的資料, Perl
提供了 Encode 這個模組, 可以讓你輕�?�地讀取及寫入舊有的編碼資料.
...
另外, 利用 encoding 模組, 你可以輕�?�寫出以字符為單位的程式碼, 如下所示:
...
請參考 Perl 內附的大量說�?�文件 (不幸全�?�用英文寫的), 來學習更多關於
Perl 的知�?, 以及 Unicode 的使用方式. 不過, 外部的資源相當豐富:
...
臺灣 Perl 連線討論區 (也就�?�各大 BBS 的 Perl 連線版)
