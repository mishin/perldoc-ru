...
Эта страница описывает самые простые шаги для понимания, создания и использования регулярных выражений ('regexes') в Perl.
...
Простейший regex это просто слово, или, более обще, строка символов.
Regex состоящее из слова соответствует любой строке, содержащей это слово:

    "Hello World" =~ /World/;  # совпадение

В этом выражении, C<World> это regex и слеши C<//> вокруг
C</World/> говорят Perl искать строку для совпадения.  Оператор
C<=~> связывает строку с regex и выдает значение true
если regex mсовпадает, или false если нет.  В нашем случае, C<World> совпадает со вторым словом в C<"Hello World">, таким образом выражение истинно. У этой идеи есть несколько вариаций.
...
Последовательность строкв regex может быть заменена переменной:
...
произвольные разделители, но тогда в начале доджны быть буква C<'m'>:
