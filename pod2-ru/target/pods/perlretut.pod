...
Хотя уже можно сделать очень много с строковый литерал
выше регулярных выражений, мы только поцарапал поверхность регулярного выражения
технология. В этом и последующих разделах мы будем вводить regexp
концепции (и связанные метасимвол нотаций), что позволит
RegExp представляют не только последовательность одного символа, но я < весь
класс настроек из них.
