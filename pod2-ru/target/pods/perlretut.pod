...
Эта страница обеспечивает базовый учебник по пониманию, созданию и
использованию регулярных выражений в Perl. Она служит в качестве дополнения к
...
Perl широко известен за выдающиеся достижения в обработке текста, и регулярные 
выражения являются одним из крупных факторов, стоящих за этой славой. Регулярные 
...
Например, когда кавычки (C<">) используются в качестве разделителя, то слеш
...
В последнем регекспе перед первым слешем C<'/'> также ставиться бэкслеш,
потому что слеш здесь используется, чтобы разграничить регексп.
...
Символ обратного слеша C<'\'> является метасимволом и для его поиска нужен еще
один обратный слеш:
...
Вот несколько примеров управляющих последовательностей:
...
Если вы уже использовали Perl какое-то время, все эти разговоры о escape-последовательностях
...
поиски регекспом почти любой символьной строкой , о которой можно мечтать.
