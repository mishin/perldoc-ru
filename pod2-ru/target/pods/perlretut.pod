...
Полуколебаний ведут себя одинаково в группах, из них: в данной
позиции строки, крайняя левая альтернатива, которая позволяет regexp для
берется матч. Так, в последнем примере на первой строки позиции,
C <"20"> соответствует второй вариант, но нет ничего левой над
чтобы соответствовать следующие две цифры C <\d\d>. Поэтому Perl переходит к следующему
альтернатива, которая является альтернативой null и что работает, так как
...
Процесс пытается один из альтернативных вариантов, видя, если он совпадает, и
Переходя к следующей альтернативой, при этом возвращаясь в строке
от где предыдущий вариант был осужден, если этого не происходит называется
Я <backtracking>. Термин «возвратом» приходит от идеи,
соответствие regexp, как прогулка в лесу. Успешно соответствие
regexp, как прибыть в пункт назначения. Существует множество возможных
trailheads, один для каждой позиции строки и каждый из них пытался в
порядке, слева направо. От каждого Трейлхед может быть много путей,
Некоторые из них получить вас там и некоторые из которых являются тупиков. Когда вы
Прогулка по тропе и попал в тупик, вам придется отступать вдоль
тропа до более ранней точки попробовать другой след. Если вы нажмете вашей
назначения, вы немедленно прекратить и забыть о попытке все
другие тропы. Ты стойким и только если вы пробовали все
трассы от всех trailheads и не прибыл в пункт назначения,
вы объявляете провал. Чтобы быть конкретным, вот пошаговый анализ
что делает Perl, когда он пытается соответствовать regexp
