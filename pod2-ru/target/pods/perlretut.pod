...
Последний пример показывает обычную задачу: сделать регистронезависимы поиск.
Perl предоставляет путь исключить все эти скобки, просто добавив
C<'i'> в конце поисковой строки. Тогда C</[yY][eE][sS]/;>
можно переписать как C</yes/i;>. C<'i'> означает
...
представлял себя и специальные символы, которые необходимо
Обратная косая черта C < \ настроек представлять себя. То же самое верно в
класс символов, но наборы обычных и специальных символов
внутри класса символов отличаются от тех, кто за пределами символа
класс. Специальные символы для класса символов являются C <-] \ ^ $настроек (и
Шаблон разделителя, какой бы она является).
C <] раздел настроек специальных, потому что оно обозначает конец класса символов. C < $ раздел настроек
Специальные, потому что оно обозначает скалярной переменной. C < \ настроек специальных потому что
Он используется в escape-последовательности, как выше. Вот как
Специальные символы C <] $\ настроек обрабатываются:
