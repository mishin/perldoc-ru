...
структуры, обсуждаемые в этом представлении состояния движка так как
автор понимает это на момент написания. 
Это B <NOT>API
определение, это чисто внутреннее руководство для тех, кто хочет взломать
regex двигателя, или понять, как работает двигатель regex. Читатели
Этот документ, как ожидается, понять синтаксис регулярных выражений perl и его
использование в деталях. Если вы хотите узнать об основах Perl
регулярные выражения, см L <perlre>. И если вы хотите заменить
engine Regex с вашим собственным, см L <perlreapi>.
...
Программа представлена массив C <regnode>структуры, один или
больше из которых представляют собой единый regop программы. Структура
C <regnode>-это маленькая структура, необходимо и имеет структуру поля, которая является
совместно с более крупные структуры.
